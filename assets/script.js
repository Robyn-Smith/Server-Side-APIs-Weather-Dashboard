...
var onSearch = () => {
    var enteredLocation = locationInput.value;//retrive location from user input
...
var displayWeather = (weatherData) => {
    document.getElementById('Country').textContent = `${weatherData.name}, ${weatherData.country}`;
...
function addSearchHistory() {
    var locallySaved = JSON.parse(localStorage.getItem('locationInput'));
    //parse turns the location back into a value

    if (locallySaved!== null) {
        recentSearch.push(...locallySaved);
        ...
        }//for loop created to add new entered location as list elements
    }
}//this retreives the data saved in local storage
...
}//find location via clicked element
